...
def add_audio(video_filename, music_filename, output_filename):
    if not os.path.exists(music_filename):
        print("\u26A0 No background music found. Skipping audio merging.")
    ...
    try:
        ...
        result = subprocess.run(command, check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        if result.returncode == 0:
            print(f"\u2705 Chess video with audio generated: {output_filename}")
            return True
        else:
...
def convert_to_short_format(input_video, output_video):
    try:
        ...
        if result.returncode == 0:
            print(f"\u2705 Video converted to Shorts format with full board: {output_video}")
            return True
        else:
